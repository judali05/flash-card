...
const flashcards: Flashcard[] = [
  ...
  { id: 2, english: "house", spanish: "casa" },
  { id: 3, english: "car", spanish: "carro" },
];
...
const Practice = () => {
    ...
    if (savedOrder) {
        ...
        const order = shuffleArray([...Array(flashcards.length).keys()]);
    ...
    if (shuffledOrder.length === 0 || shuffledOrder[currentIndex] === undefined) {
        return <div className="text-center mt-10">Cargando...</div>;
    }

    const currentCard = flashcards[shuffledOrder[currentIndex]];
    ...
    if (nextIndex >= flashcards.length) {
        ...
        const newOrder = shuffleArray([...Array(flashcards.length).keys()]);
    ...
    // Calcula progreso (coloca esto dentro del componente, antes del return)
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-white px-4">
            ...
            {currentIndex >= flashcards.length - 1 && showFeedback && (
                <button
                    onClick={() => {
                    const newOrder = shuffleArray([...Array(flashcards.length).keys()]);
                    setShuffledOrder(newOrder);
